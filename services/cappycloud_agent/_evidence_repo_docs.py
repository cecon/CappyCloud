"""Repository document and graph evidence lookup."""
...
from ._evidence_models import _DocHit
from ._evidence_utils import _dedupe, _trim
...
async def _fetch_repo_docs(
    ...
    for query in initial_queries:
        try:
            hits.extend(await _fetch_repo_graph_for(client, session_url, repo_ids, query))
        except Exception:
            continue
    for query in initial_queries:
        try:
            hits.extend(await _fetch_repo_docs_for(client, session_url, repo_ids, query))
        except Exception:
            continue
    ...
    for query in _repo_doc_expansion_queries(_unique_docs(hits), user_message):
        ...
        try:
            expansion_hits.extend(await _fetch_repo_docs_for(client, session_url, repo_ids, query))
        except Exception:
            continue
...
def _repo_doc_queries(terms: list[str], user_message: str) -> list[str]:
    ...
    return [query for query in _dedupe(candidates) if query and len(query.strip()) >= 3][
...
def _table_heading_match(summary: str, table: str) -> re.Match[str] | None:
    return re.search(rf"^####\s+{re.escape(table)}\b.*$", summary, re.IGNORECASE | re.MULTILINE)
