...
from app.domain.entities import Repository, UserRole
...
class TestStreamMessage:
    ...
    async def test_starts_stream_with_flush_comment(
        ...
        assert "Conexão com o agente aberta.".encode("utf-8") in second
    ...
    async def test_sends_heartbeat_while_waiting_for_agent_chunk(
        ...
        conv = await CreateConversation(conv_repo).execute(user_id, "Chat")
        uc = StreamMessage(conv_repo, msg_repo, _SlowAgent(delay_s=0.05))
        stream = await uc.execute(conv.id, user_id, "Olá agente")
    ...
    async def test_resolves_effective_model_before_streaming(
        ...
        uc = StreamMessage(conv_repo, msg_repo, agent, model_access=policy)
