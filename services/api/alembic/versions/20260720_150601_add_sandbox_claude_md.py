"""add sandbox claude md

Revision ID: b6fbc5d81584
Revises: 4d90cbbafcbd
Create Date: 2026-07-20 15:06:01.615265
...
DEFAULT_CLAUDE_MD = """# Sandbox — Global Skills Reference
...
revision: str = "b6fbc5d81584"
down_revision: str | Sequence[str] | None = "4d90cbbafcbd"
...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            """
            UPDATE sandboxes
            SET claude_md = :content
            WHERE COALESCE(claude_md, '') = ''
            """
        ).bindparams(content=DEFAULT_CLAUDE_MD)
    )


def downgrade() -> None:
    op.drop_column("sandboxes", "claude_md")
