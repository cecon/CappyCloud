...
| `multimodal-grpc-handler.patch` | `services/sandbox/patches/multimodal-grpc-handler.patch` | shimmed for v0.24 | The legacy patch still produces rejects against v0.24.0, but `services/sandbox/patch_openclaude_grpc_v024.js`, `services/sandbox/cappycloud_grpc_helpers_v024.ts` and `services/sandbox/cappycloud_grpc_diagnostics_v024.ts` now restore the required imports/helpers explicitly before build. Runtime inspection confirms `cappycloudGrpcString`, `cappycloudValidateToolScope`, `cappycloudPayloadDiagnostic` and MCP command loading are present after `env_init`. |
...
- Production push/deploy: verify the tolerated rejects before production rollout and rely on the image workflow for deployment.
